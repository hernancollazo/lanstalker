...
def get_all_known_macs():
    return [host.mac for host in Host.query.filter(Host.mac.isnot(None)).all()]


def save_host_history(host_id, ip=None, hostname=None):
    entry = HostHistory(host_id=host_id, ip=ip, hostname=hostname)
...
def log_change(mac, change_type, old_ip=None, new_ip=None):
    log = ChangeLog(mac=mac, old_ip=old_ip, new_ip=new_ip, change_type=change_type)
    db.session.add(log)
    db.session.commit()
